...
// postDeadline과 오늘 날짜를 비교하여 오늘로부터 7일 이내인지 여부를 반환하는 함수
export const isWithin7Days = (postDeadlineTimestamp: Timestamp): boolean => {
  const postDeadline: Dayjs = timestampToDayjs(postDeadlineTimestamp);
  const today: Dayjs = getToday();

  // postDeadline이 오늘보다 이후이고, 오늘로부터 7일 이내인 경우 true 반환
  return postDeadline.isAfter(today) && postDeadline.diff(today, 'day') <= 7;
};
